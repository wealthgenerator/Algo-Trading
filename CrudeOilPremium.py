...
Month = "NOV" ; year=23; interval = "10minute"; dir_ = "CRUDE_DATA/%s"%Month

print("Session started---------------------")
month_list = ['JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC']
stock="CRUDEOILM";  index = month_list.index(Month); N_Month = month_list[index+1 if (index+1)<=11 else (index+1)-12];  N_N_Month = month_list[index+2 if (index+2)<=11 else (index+2)-12 ]
...
for i, Month in enumerate(data_list):
    ...
    if days[i]>0: #if no of days is greater than 0 for given month then only ask for hostorical data 
        instrument = "MCX:%s%s%sFUT"%(stock,year_list[i],Month)
...
df3.to_csv("%s/CRUDEOILM_%s%s_%s.csv"%(dir_,data_list[0],year,interval))

plt.plot(data_N["date"],P)# .value givea numy array. otheriwse you will have index value storred. 
plt.savefig("%s/CRUDEOILM_premium_%s_%s.png"%(dir_,data_list[0],interval))
